...
RUN_PREFIX = "single_queue_run_"

# 随机车流生成脚本部署在共享 envs 中，这里只引用它，不在模块内复制环境代码。
# The random traffic generator is deployed in the shared envs folder and is imported here without duplicating env code.
...
def train(run_idx: Optional[int] = RUN_IDX) -> None:
    ...
    # 当前基础版本使用固定学习率 3e-5，便于和后续实验保持清晰边界。
    # This base version uses a fixed learning rate of 3e-5 to keep a clear boundary from later experiments.
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=3e-5,
