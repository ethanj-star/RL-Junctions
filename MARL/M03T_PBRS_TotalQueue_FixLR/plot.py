...
# RUN_IDX 用于选择要绘制的 total-queue PBRS 训练日志，WINDOW_SIZE 控制曲线平滑窗口。
# RUN_IDX selects the total-queue PBRS training logs to plot, and WINDOW_SIZE controls the smoothing window.
...
def plot_learning_curve() -> None:
    ...
    axes[0].set_title(f"MARL total-queue PBRS fixed LR waiting time, run {RUN_IDX}")
    axes[0].legend()
