...
RUN_PREFIX = "multi_queue_fix_lr_run_"
...
def train(run_idx: Optional[int] = RUN_IDX) -> None:
    ...
    # 当前基础版本使用固定学习率 3e-5，作为 MARL queue reward 的对照组。
    # This base version uses a fixed learning rate of 3e-5 as the MARL queue-reward reference.
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=3e-5,
