...
RUN_PREFIX = "fixlr_m03a_active_phase_run_"
...
# 创建 PettingZoo MARL 环境，三个路口分别作为 agent，reward 使用 PBRS。
# This creates the PettingZoo MARL environment where each intersection is an agent using PBRS reward.
...
def train(run_idx: Optional[int] = RUN_IDX) -> None:
    ...
    # 当前版本使用 active-phase PBRS 和固定学习率 3e-5。
    # This version uses active-phase PBRS with a fixed learning rate of 3e-5.
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=3e-5,
