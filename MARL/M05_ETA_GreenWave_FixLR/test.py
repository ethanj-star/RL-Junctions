...
# RUN_IDX 选择模型编号，USE_GUI 控制是否打开 SUMO 界面。
# RUN_IDX selects the model, and USE_GUI controls the SUMO interface.
RUN_IDX = int(os.environ.get("JUC_RUN_IDX", "1"))
USE_GUI = os.environ.get("JUC_USE_GUI", "0") == "1"


# 加载指定模型和归一化状态，并执行一个完整的确定性演示回合。
# This loads the selected model and normalization state, then runs one deterministic episode.
def run_test() -> None:
    run_dir = MODELS_DIR / f"{RUN_PREFIX}{RUN_IDX}"
    model_path = run_dir / "ppo_marl_model_bidirectional_eta.zip"
    vec_path = run_dir / "vec_normalize_marl_bidirectional_eta.pkl"
