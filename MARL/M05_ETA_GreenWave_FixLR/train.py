...
# 奖励由总排队 PBRS、双向 ETA 对齐、自由流、支路公平和回溢保护组成。
# The reward combines total-queue PBRS, bidirectional ETA alignment, free flow, fairness, and spillback protection.
def green_wave_reward(traffic_signal) -> float:
    ...
    if is_main_green:
        ...
        if arrival_pressure < 0.2 and side_queue > 0.0:
            idle_penalty = -IDLE_MAIN_GREEN_PENALTY
        if side_queue > SIDE_QUEUE_THRESHOLD and green_elapsed > LONG_MAIN_GREEN_START:
            ...
            fairness_penalty = -SIDE_QUEUE_WEIGHT * overload * duration
...
# 观测在 SUMO-RL 默认维度后追加 18 维邻居、双向 ETA 和本地控制特征。
# The observation appends 18 neighbor, bidirectional ETA, and local-control features.
def get_observation_class():
    ...
    class BidirectionalETAObservationFunction(DefaultObservationFunction):
        def observation_space(self):
            ...
            return spaces.Box(
                low=np.zeros(base_dim + 18, dtype=np.float32),
                high=np.full(base_dim + 18, np.inf, dtype=np.float32),
            )
        ...
        def __call__(self):
            ...
            return np.concatenate([super().__call__(), np.asarray(extra, dtype=np.float32)]).astype(np.float32)

    return BidirectionalETAObservationFunction


# 每个 episode 重生成随机车流，并构建与 PPO 兼容的三智能体向量环境。
# Random traffic is regenerated per episode before building the PPO-compatible three-agent vector environment.
...
def train(run_idx: Optional[int] = RUN_IDX) -> None:
    ...
    model.save(str(model_dir / "ppo_marl_model_bidirectional_eta"))
    env.save(str(model_dir / "vec_normalize_marl_bidirectional_eta.pkl"))
    env.close()
