...
# RUN_IDX 选择训练日志，WINDOW_SIZE 控制移动统计窗口。
# RUN_IDX selects the training logs, and WINDOW_SIZE controls the rolling window.
...
# 读取当前 run 的 SUMO CSV，并按 episode 汇总等待时间和排队长度。
...
def load_episode_stats() -> Optional[pd.DataFrame]:
    ...
    for csv_path in sorted(LOG_DIR.glob("*.csv")):
        ...
        frame["episode"] = int(match.group(1))
        frames.append(frame)
    ...
    ).reset_index().sort_values("episode")
...
# 绘制等待时间与排队长度曲线，并保存到当前 run 的日志目录。
...
# This plots waiting-time and queue-length curves and saves them in the selected run directory.
def plot_learning_curve() -> None:
    ...
    axes[0].set_title(f"MARL PBRS communication green-wave waiting time, run {RUN_IDX}")
    axes[0].legend()
