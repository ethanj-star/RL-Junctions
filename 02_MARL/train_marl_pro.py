...
if __name__ == '__main__':
    ...
    model = PPO(
        ...
        env,
        learning_rate=linear_schedule_with_min(3e-4, 3e-5),  # 学习率：优化器的更新步长。linear decrease to 3e-5
        n_steps=2048,  # 截断步数（Rollout Steps）：每次策略更新前，每个并行环境收集的交互步数。总缓冲区大小等于该值乘以环境总数。
        batch_size=256,  # 微批次大小（Mini-batch Size）：每次执行随机梯度下降（SGD）计算策略梯度时的样本量。
        n_epochs=10,  # 优化轮数：对单次收集的经验回放缓冲区（Buffer）数据进行反复复习的遍数。降低此值可防止模型对当前批次经验产生过拟合。
        clip_range=0.2,  # 裁剪参数（Clipping Parameter）：PPO算法的核心超参数，限制新旧策略比值在 [0.8, 1.2] 区间内，保证信任域（Trust Region）内的策略更新单调性。
        ent_coef=0.03,  # 熵系数（Entropy Coefficient）：AI 的好奇心。0.01 表示保留一丁点尝试新鲜动作的欲望，防止模型过早收敛至局部最优策略。
